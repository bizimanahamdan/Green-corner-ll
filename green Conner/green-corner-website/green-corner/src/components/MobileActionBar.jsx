...
import { isConfirmedPhone, isConfirmedWhatsApp, telHref } from "../lib/business";
...
export default function MobileActionBar() {
  ...
  const cols = 1 + (hasPhone ? 1 : 0) + (hasWhatsApp ? 1 : 0);

  return (
    <nav
      className="sm:hidden fixed bottom-0 left-0 right-0 z-40 bg-char-950 border-t border-white/10"
      style={{ paddingBottom: "env(safe-area-inset-bottom, 0px)" }}
      aria-label="Quick actions"
    >
      <div className={`grid ${cols === 3 ? "grid-cols-3" : cols === 2 ? "grid-cols-2" : "grid-cols-1"}`}>
        {hasPhone && (
          <a
            href={phoneHref}
            className="flex flex-col items-center justify-center gap-0.5 py-2.5 text-paper/80 active:bg-white/5 min-h-[56px]"
          >
        ...
        {hasWhatsApp && (
          <button
            onClick={openModal}
            className="flex flex-col items-center justify-center gap-0.5 py-2.5 text-leaf-400 border-x border-white/10 active:bg-char-900 min-h-[56px]"
          >
        ...
        {count > 0 ? (
          <button
            type="button"
            onClick={openCart}
            className="flex flex-col items-center justify-center gap-0.5 py-2.5 text-citrus-600 active:bg-citrus-500/10 min-h-[56px]"
          >
            <span className="relative">
              ...
              <span className="absolute -top-2 -right-3 rounded-full bg-citrus-500 px-1 text-[10px] font-bold text-char-950">
                {count}
              </span>
            </span>

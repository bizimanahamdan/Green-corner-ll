...
export default function MobileActionBar() {
  ...
  return (
    <nav
      className="sm:hidden fixed bottom-0 left-0 right-0 z-40 bg-char-950 border-t border-line/10"
    ...
    >
      <div className={`grid ${cols === 3 ? "grid-cols-3" : cols === 2 ? "grid-cols-2" : "grid-cols-1"}`}>
        ...
        {hasWhatsApp && (
          <button
            onClick={openModal}
            className="flex flex-col items-center justify-center gap-0.5 py-2.5 text-leaf-400 border-x border-line/10 active:bg-char-900 min-h-[56px]"
          >
        ...
        {count > 0 ? (
          ...
          >
            <span className="relative">
              ...
              <span className="absolute -top-2 -right-3 rounded-full bg-citrus-500 px-1 text-[10px] font-bold text-soot">
                {count}
              </span>
            </span>

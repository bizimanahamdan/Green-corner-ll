            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">
              {visible.map((img) => (
                <figure key={img.id} className="card-surface overflow-hidden">
                  <div className={`aspect-square flex items-center justify-center ${isIllustration(img.url) ? "bg-char-800 p-8" : ""}`}>
                    <img
                      src={img.url}
                  ...
                  {img.caption && (
                    <figcaption className="p-3 text-xs text-mute">{img.caption}</figcaption>
                  )}
                </figure>
              ))}
            </div>
